...
pg *remocao(pg *pai, pg *raiz ,int val, int pos){
    if(pagina == NULL)
        return;
    int i;
    for(i = 0; i < raiz->nchaves && raiz->chaves[i] < val; i++);

    if(i < raiz->nchaves && raiz->chaves[i] != val){
		remocao(raiz->filhos[i], val);
		if(raiz->nchaves > ceil(ordem/2.0))
			return NULL;
		redistribuir(pai,)
		concatenacao(pai, i);
	}
    return remocao(pai,pagina->filhos[i], val, i);
}

void concatenaRaiz(pg **raiz){
    pg *aux;
    if(*raiz == NULL)
        return;
    if((*raiz)->nchaves == 0 && (*raiz)->filhos[0] != NULL){
        aux = *raiz;
        *raiz = (*raiz)->filhos[0];
        free(aux);
    }
